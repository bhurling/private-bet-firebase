...
class BeanRegistry {
     ...
     */
    private fun getScopeForClass(clazz: KClass<*>) = definitions[definitions.keys.first {
        it.clazz == clazz || it.bindTypes.contains(clazz)
    }]
     ...
     */
    fun searchByName(clazz: KClass<*>,
                     name: String): BeanDefinition<*> = searchDefinition { it.clazz == clazz && it.name == name }.firstOrNull()
            ?: throw NoBeanDefFoundException("No bean definition found for name $name and class $clazz")
    ...
    fun searchAll(clazz: kotlin.reflect.KClass<*>): BeanDefinition<*> {
        ...
        return when {
            found.size > 1 -> throw NoBeanDefFoundException(
                    "Multiple definition found for class $clazz : \n\t$concreteTypes\n\t$extraBindTypes")
    ...
    private fun searchDefinition(
            filter: (BeanDefinition<*>) -> Boolean): List<BeanDefinition<*>> = definitions.keys.filter(filter)
